...
筛选条件、详情页补充范围和输出；**用户本次描述优先于 rules.md，rules.md 只是未指定参数
的默认值**——国家、品类、榜单、时间、数量都可由用户临时指定，不写死。

## 流程

0. **读取前置规则**：`rules.md` 提供默认值（筛选条件、详情页补充 Top N、标签阈值、
   抓取参数）；先用用户本次需求覆盖对应参数，再按剩余默认值执行。
