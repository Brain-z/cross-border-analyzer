...
def img_cell(r):
    ...
    if not url:
        return "-"
    name = (r.get("name") or "商品图片").replace("|", "\\|")
    return f"![{name[:20]}]({url})"
...
def main():
    ...
    lines.append("")
    lines.append("| # | 图片 | 商品 | 价格($) | 周期销量 | 佣金率 | 热度 | 可做性 | 潜力 | 标记 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    ...
    for i, r in enumerate(top, 1):
        ...
        lines.append(f"| {i} | {img_cell(r)} | {name} | {fmt_num(price)} | {fmt_num(s7)} | "
    ...
    for i, r in enumerate(sorted(rows, key=lambda x: f(x.get("viability")) or 0,
        ...
        lines.append(f"| {i} | {img_cell(r)} | {name} | {fmt_num(price)} | "
                     f"{fmt_num(via)} | {'、'.join(reasons)} |")
    lines.append("")
    ...
    lines.append("- 图片为 FastMoss 缩略图，打开报告时联网加载；无图片时显示 `-`；")
